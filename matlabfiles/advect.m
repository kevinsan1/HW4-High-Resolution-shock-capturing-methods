...
ip = (1:N)+1;  %ip(N) = N-1;   % ip = i+1 with periodic b.c.
im = (1:N)-1; % im(1) = 2;   % im = i-1 with periodic b.c.
h = padarray(h',1,0)'
m = padarray(m',1,0)'
h(1) = h(2);
m(1) = -m(2);
...
for iStep=1:nStep  %% MAIN LOOP %%
    ...
    flux(nonghost) = [m(nonghost) ; 1/2*g*h(nonghost).^2 + m(nonghost).^2/h(nonghost)];
